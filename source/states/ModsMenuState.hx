...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(modsList.all.length < 1)
		{
			...
			var txt = new FlxText(bgList.x + 15, bgList.y + 15, bgList.width - 30, LanguageBasic.getPhrase('no_mods_found', "No Mods found."), 16);
		...
		var myHeight = 100;
		modRestartText = new FlxText(bgDescription.x + 15, bgDescription.y + bgDescription.height - myHeight - 25, bgDescription.width - 30, LanguageBasic.getPhrase('mod_restart', '* Moving or Toggling On/Off this Mod will restart the game.'), 16);
		...
		var bottomText = new FlxText(bottomBG.x, bottomBG.y + 4, FlxG.width, LanguageBasic.getPhrase('mods_leave', "Press {1} To Leave", [daButton]), 16);
